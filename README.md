# Colour Recognition App (Python + Streamlit)

_This project is a simple Colour Recognition application built with Python and Streamlit. The goal is to detect the colour of any pixel in an image and display its RGB values along with the closest colour name from a CSV dataset._

## Features

- _Upload any image (JPG or PNG)_
- _Click on a pixel (or select coordinates via slider) to get RGB values_
- _Get the closest colour name from a CSV of over 1000 colours_
- _Works in a web browser using Streamlit_
- _No machine learning model required – uses a simple RGB lookup_

